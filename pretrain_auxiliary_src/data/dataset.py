...
class ReverieTextPathData(object):
    ...
    def get_input(
        ...
        # print('traj_obj_img_fts:',len(traj_obj_img_fts))
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            ...
            # print('view_fts:',view_fts,view_fts.shape)
...
class R2RTextPathData(ReverieTextPathData):
    ...
    def get_scanvp_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)
        if self.in_memory and key in self._feature_store:
            view_fts, obj_attrs = self._feature_store[key]
        else:
            with h5py.File(self.img_ft_file, 'r') as f:
                view_fts = f[key][...].astype(np.float32)
                    ...
                    if key in f:
                        for attr_key, attr_value in f[key].attrs.items():
                            if attr_key in ['directions', 'sizes', 'bboxes', 'obj_ids']:
                                obj_attrs[attr_key] = attr_value[:self.max_objects]
            if self.in_memory:
                self._feature_store[key] = (view_fts, obj_attrs)

        return view_fts, obj_attrs
    ...
    def get_input(
            self, idx, end_vp_type, return_img_probs=False, return_act_label=False, return_obj_label=False, end_vp=None
    ):
        item = self.data[idx]
        scan = item['scan']
        start_vp = item['path'][0]
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            view_fts,obj_attrs = self.get_scanvp_feature(scan, vp)
        ...
        return traj_view_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, last_vp_angles,last_vp_objids
