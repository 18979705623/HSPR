...
from suat.reg_prox_knowl import construct_reg_prox
from suat.obj_pro_knowl import construct_obj_prox
from suat.reg_obj_prox_knowl import construct_rej_obj_prox
...
def create_dataloaders(
    ...
    for k, task_name in enumerate(data_cfg.tasks):
        if task_name == 'mlm':
            ...
            task_dataset = RCDataset(nav_db, tok)
            task_collate_fn = rc_collate
        elif task_name == 'mrc':
...
def main(opts):
    ...
    LOGGER.info("Initially constructing the hierarchical spatial proximity knowledge base...")
    construct_reg_prox()
    construct_obj_prox()
    construct_rej_obj_prox()
...
def validate(model, val_dataloaders, setname=''):
    ...
    for task, loader in val_dataloaders.items():
        ...
        if task.startswith('mlm'):
            ...
            val_log = validate_rc(model, loader)
...
@torch.no_grad()
def validate_rc(model, val_loader):
    LOGGER.info("start running RC validation...")
    val_loss, n_feat, n_correct = 0, 0, 0
    st = time.time()
    for i, batch in enumerate(val_loader):
        view_logits, view_targets = model(batch, task='rc', compute_loss=False)
        val_loss += F.cross_entropy(view_logits, view_targets, reduction='sum').data.item()
        n_correct += torch.sum(torch.argmax(view_logits, 1) == view_targets).item()
        n_feat += sum(batch['traj_step_lens'])
    val_loss = sum(all_gather(val_loss))
    n_correct = sum(all_gather(n_correct))
