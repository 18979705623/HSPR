...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_obj_labels = self.compute_label_for_point([ob['obj_labels'] for ob in obs])
        ...
        return {
            ...
            'obj_labels': batch_obj_labels, 'room_type': batch_room_type,
    ...
    def _nav_vp_variable(self, obs, gmaps, pano_embeds, conn, cand_vpids, view_lens, obj_lens, nav_types,
                         end_room_type):
        batch_size = len(obs)
                    ...
                    if self.args.detailed_output:
                        for k, v in gmaps[i].node_stop_scores.items():
                            traj[i]['details'][k] = {
                                ...
                                'obj_ids': [int(x) for x in v['og_details']['objids']],
