...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        self.label_set = json.load(open('../datasets/labels/all_labels.json'))
        self.room_label = json.load(open('../datasets/labels/house_pano_info.json'))
    ...
    def get_view_labels(self, world_state):
        query = '_'.join([world_state.scanId,
                          world_state.location.viewpointId])
        result = []
        for i in range(0, 36):
            query1 = query + f'_{i}'
            try:
...
class ReverieObjectNavBatch(object):
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ob = {
                ...
                'obj_labels': self.env.get_view_labels(state),
                'room_type': self.env.room_label[state.scanId][state.location.viewpointId],
                'end_room_type': self.env.room_label[state.scanId][item['path'][-1]]
            }
    ...
    def eval_metrics(self, preds):
        ...
        for item in preds:
            ...
            # print('pred_objid:', pred_objid, type(pred_objid))
