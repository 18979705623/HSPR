...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def get_room_type_for_cut(self, obs):
        current_vpids = []
        for ob in obs:
            current_vpids.append(ob['viewpoint'])
        room_list = torch.LongTensor([])
        for i, (ob, vpd) in enumerate(zip(obs, current_vpids)):
            room_type = torch.LongTensor([self.env.room_label[ob['scan']][vpd]])  # int——>tensor
    ...
    def get_room_type_for_gmap_node(self, obs, cand_vips):
        ...
        for i, cand_vip in enumerate(cand_vips):
            max_num_a = max(max_num_a, len(cand_vip))
        for i, (ob, cand_vip) in enumerate(zip(obs, cand_vips)):
            list_view = torch.LongTensor([])
            for viewpointId in cand_vip:
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_view_lens, batch_obj_lens, batch_cand_vpids = [], [], []
        batch_end_room_type = []
        batch_size = len(obs)
        ...
        batch_room_type = self.get_room_type_for_view(obs).reshape(batch_size, -1)
        bath_current_room_type = self.get_room_type_for_cut(obs)
        batch_end_room_type = torch.LongTensor(batch_end_room_type).cuda()
        batch_view_obj_labels = self.compute_label_for_action(obs)

        return {
            'view_img_fts': batch_view_img_fts, 'obj_img_fts': batch_obj_img_fts,
            'loc_fts': batch_loc_fts,
            'nav_types': batch_nav_types, 'view_lens': batch_view_lens, 'obj_lens': batch_obj_lens,
            'cand_vpids': batch_cand_vpids, 'room_type': batch_room_type,
            'current_room_type': bath_current_room_type, 'end_room_type': batch_end_room_type,
            'view_obj_labels': batch_view_obj_labels
        }
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        batch_no_vp_left, batch_gmap_node_viewpoint = [], []
        for i, gmap in enumerate(gmaps):
            ...
            gmap_node_viewpoint = gmap.get_node_viewpoint()
            ...
            batch_gmap_node_viewpoint.append(gmap_node_viewpoint)
        ...
        nv_node_room_type = self.get_room_type_for_gmap_node(obs, batch_gmap_node_viewpoint).reshape(batch_size, -1)
        return {
            ...
            'no_vp_left': batch_no_vp_left, 'nv_node_room_type': nv_node_room_type
        }

    def _nav_vp_variable(self, obs, gmaps, pano_embeds, conn, cand_vpids, view_lens, nav_types, end_room_type):
        batch_size = len(obs)
        ...
        for t in range(self.args.max_action_len):
            ...
            nav_inputs.update(
                self._nav_vp_variable(
                    ...
                    pano_inputs['view_lens'], pano_inputs['nav_types'], pano_inputs['end_room_type']
                )
            )
            nav_inputs.update({
                'txt_embeds': txt_embeds,
                'txt_masks': language_inputs['txt_masks'],
                'current_room_type': pano_inputs['current_room_type']
            })
            ...
            for i in range(batch_size):
                if (not ended[i]) and just_ended[i]:
                    ...
                    if stop_node is not None and obs[i]['viewpoint'] != stop_node:
                        for vp in gmaps[i].graph.path(obs[i]['viewpoint'], stop_node):
                            traj[i]['path'].append([vp])
